...
function selectChord(chord) {
  ...
  if (selectedchord == chord) {
    ...
    $('#chordpiano').klavier(
      'setSelectedValues',
      noteArraytoMidi(sessionchords[chord][0])
    );
...
function playChord(input) {
  if (input == null) {
    rhythminstrument.triggerAttackRelease(
      ...
      sessionchords[selectedchord][1] * Tone.Time('1m')
    );
    isPlayingChord = true;
  ...
  $('#chordpiano').klavier(
    'setSelectedValues',
    noteArraytoMidi(scalechords[input])
  );
...
function getChordsFromScale() {
  ...
  for (var x = 0; x < 7; x++) {
    ...
    scalechords.push(
      Tone.Frequency(notes[chordsroot].split('/')[0] + '4').harmonize(thischord)
    );
...
function removeChord() {
  ...
  var chordindexinmeasure = chordsinthismeasure.indexOf(
    sessionchords[selectedchord]
  );
  ...
  if (siblingchordsnum == 1) {
    ...
    if (chordindexinmeasure == 0) {
      ...
      sessionchords[selectedchord + 1][3] = sessionchords[
        selectedchord
      ][3].concat(sessionchords[selectedchord + 1][3]);
    } else {
      sessionchords[selectedchord - 1][1] += sessionchords[selectedchord][1];
      sessionchords[selectedchord - 1][3] = sessionchords[
        selectedchord - 1
      ][3].concat(sessionchords[selectedchord][3]);
    }
...
function generateRandomProgression() {
  ...
  for (var z = 0; z < sessionlength; z++) {
    ...
    for (var x = 0; x < chordsinthismeasure; x++) {
      ...
      thischordnotes = Tone.Frequency(
        notes[chordsroot].split('/')[0] + '4'
      ).harmonize(thischordnotes);
...
function setRhythmInstrument() {
  ...
  if (instruments[rhythmpatch].base == 'Sampler') {
    ...
    rhythminstrumentbuffers = new Tone.Buffers(
      instruments[rhythmpatch].urls,
      instruments[rhythmpatch].options
    );
  }
...
function drawRhythm() {
  ...
  var measurechords = sessionchords.filter(
    (chord) => chord[2] == playbackMeasure + 1
  );
  ...
  measurechords.forEach((chord, chordindex) => {
    var rechordlbl =
      ...
      chordindex +
      '" class="re-chname">' +
      chordNotestoName(chord[0]) +
      '</span>' +
      $('#rhythmeditor').append(rechordlbl);
  });

  $('#rt-' + (sessionsubdivision - 1)).css(
    'border-right',
    'solid 1px var(--darkest-color)'
  );
...
function editRhythm(chord, add_delete) {
  ...
  var chordindexes = [];
  sessionchords.forEach((e, i) => {
    if (e[2] == measure) chordindexes.push(i);
  });
...
$('#floatinginput').blur(function (e) {
  $('#chord' + (changingnamechord + 1)).html(
    chordNotestoName(sessionchords[changingnamechord][0])
  );
  $('#floatinginput')
    .removeClass('visible')
    .addClass('hidden')
...
$('#floatinginput').change(function (e) {
  sessionchords[changingnamechord][0] = chordNametoNotes(
    $('#floatinginput').val()
  );
  $('#chord' + (changingnamechord + 1)).html(
    chordNotestoName(sessionchords[changingnamechord][0])
...
$('#chordpiano').click((e) => {
  ...
  if (pianonotes.indexOf(notepressed) != -1) {
    rhythminstrument.triggerAttackRelease(
      Tone.Frequency(notepressed, 'midi'),
      '4n'
    );
  }
...
$('.chordbtn').draggable({
  ...
  drag: function (event, ui) {
    if (isChordHovered) {
      var innerposition =
        ui.offset.left - $('#chord' + hoveredchord).offset().left;
      hoveredside =
        innerposition < $('#chord' + hoveredchord).width() / 2 - 27 ? 0 : 1;
      ...
      if (sessionchords[hoveredchord - 1][1] > 1 / maxchordspermeasure) {
        ...
        $('#addchordhelper')
          ...
          .css({
            ...
            left:
              $('#chord' + hoveredchord).offset().left +
              (hoveredside == 0 ? 0 : $('#chord' + hoveredchord).width() / 2),
...
$(document).on('click', '.re-tile', (e) => {
  ...
  sessionrhythm[playbackMeasure][index] =
    sessionrhythm[playbackMeasure][index] == 0 ? 1 : 0;
  sessionrhythm[playbackMeasure][index + 1] == null &&
  index + 1 < sessionsubdivision
    ? (sessionrhythm[playbackMeasure][index + 1] = 1)
    : '';
  updateRhythm();
  onModifySession();
});
