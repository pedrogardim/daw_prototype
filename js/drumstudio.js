...
function drawDrumKeys() {
  ...
  drumSounds.forEach((e, i) => {
    var thislabel = seldrumkit.hasOwnProperty('labels')
      ? seldrumkit.labels[i]
      : drumelemcategory[seldrumkit.elemcat[i]][0];
    var thisicon = seldrumkit.hasOwnProperty('icons')
      ? seldrumkit.icons[i]
      : drumelemcategory[seldrumkit.elemcat[i]][1];

    drumkeys +=
      '<div class="drumkey" data-index="' +
      i +
      '" data-trigger="' +
      drumtriggers[1] +
      '">' +
      ...
      thislabel +
      '</span>' +
      '<svg height="48px" width="48px" viewBox="0 0 64 64">' +
      thisicon +
      '</svg>' +
...
function playDrumSound(keycodetoindex) {
  if (keycodetoindex < drumSounds.length) {
    ...
    $('.drumkey[data-index="' + keycodetoindex + '"]').css(
      'background-color',
      'var(--medium-color)'
    );
  }
...
function drawSequencer() {
  ...
  for (var y = 0; y < drumSounds.length; y++) {
    ...
    var elementicon = seldrumkit.hasOwnProperty('icons')
      ? seldrumkit.icons[y]
      : drumelemcategory[seldrumkit.elemcat[y]][1];

    var thisicon =
      '<svg height="40px" width="40px" viewBox="0 0 64 64" class="ce" id="seqlblel' +
      (y + 1) +
      '">' +
      elementicon +
      '</svg>';
    ...
    var instrlabel =
      '<span class="seqlbl hidden" id="seqlbl' +
      (y + 1) +
      '">' +
      drumlabels[y] +
      '</span>';
...
function updateMsreScroreTiles() {
  ...
  sessiondrums.forEach((msre, msreindex) => {
    ...
    existentnotesonsession.forEach((note, noteindex) => {
      var thisrow =
        '<div class="drummsrerow" style="order:' +
        note +
        '" id="drummsrerow-' +
        msreindex +
        '-' +
      ...
      msre.forEach((beat, beatindex) => {
        ...
        if (beat.indexOf(note) != -1) {
          $('#mt' + msreindex + '-' + beatindex + '-' + note).addClass(
            'activetile'
          );
        }
      });
...
function registerNoteToSequencer(note, beat) {
  ...
  if (sessiondrums[playbackMeasure][beat].includes(note) == true) {
    sessiondrums[playbackMeasure][beat] = sessiondrums[playbackMeasure][
      beat
    ].filter((e) => e != note);
...
function adaptSessiontoSubdiv() {
  sessiondrums = sessiondrums.map((e) => {
    return adaptSequencetoSubdiv(e);
  });

  sessionrhythm = sessionrhythm.map((e) => {
    return adaptSequencetoSubdiv(e);
  });
...
$('#seq-steps-input').on('change', function (e) {
  var goal = $(this).val();
  var closest = stepvalues.reduce((prev, curr) => {
...
function updateTiles() {
  ...
  sessiondrums[playbackMeasure].forEach(function (beat, btindex) {
    beat.forEach(function (element, index) {
      $('#seqTile-' + btindex + '-' + (element - 1)).toggleClass(
        'selectedTile'
      );
    });
  });
...
function updateSequencerElements() {
  ...
  $('#drummeasure' + (playbackMeasure + 1))
    .addClass('hightlightedmeasure')
...
$(document).on('mousemove', '.seqtile', function (e) {
  ...
  if (dragtileselect) {
    if (
      ($(this).hasClass('selectedTile') && dtsmode == 'remove') ||
      ($(this).hasClass('selectedTile') == false && dtsmode == 'add')
    ) {
      $(this).toggleClass('selectedTile');
  ...
  if (appMode == 2) {
    ...
    if (drumview == 1) {
      ...
      if (e.keyCode == 37 || e.keyCode == 39) {
        if (e.keyCode == 37 && playbackBeat > 0) {
          playbackBeat--;
        } else if (
          e.keyCode == 37 &&
          playbackBeat == 0 &&
          ...
          playbackMeasure--;
          playbackBeat = sessiondrums[playbackMeasure].length - 1;
        } else if (
          e.keyCode == 39 &&
          playbackBeat < sessiondrums[playbackMeasure].length - 1
        ) {
          ...
          e.keyCode == 39 &&
          playbackBeat == sessiondrums[playbackMeasure].length - 1 &&
          playbackMeasure < sessiondrums.length - 1
        ) {
          playbackMeasure++;
          playbackBeat = 0;
        }
